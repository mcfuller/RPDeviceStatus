 ...
 * 				commands from HDX code. Unfortunately, because the REST API wasn't designed with a
 * 				standardized command syntax and because the Group Series did not implement the HDX's awesome
 * 				"/a_apicommand.cgi?apicommand=" interface, each command would essentially be a special
 * 				case to be tested for, making a doApiCommand() method a very tedious undertaking that
 * 				would require constant maintenance as new commands are added in subsequent software
 * 				releases. No thank you. There was talk of implementing the HDX's cgi?apicommand= interface
 * 				for the Group Series, (which would make sense since most of the commands are identical
 * 				when using the command shell via telnet) but I'm not holding my breath...
 * 
	...
	public class GroupSeriesDevice : RPDevice
	{
		...
		public override bool getSession()
		{
			...
			if (string.IsNullOrEmpty(sessionID))
			{
				...
				// Send finished command, report all output
				try
				{
					// Post data to stream
		...
		public override List<string> getStatus()
		{
			...
			List<GroupSeriesJSONObjects.statusJSONObject> response = new List<GroupSeriesJSONObjects.statusJSONObject>();
			try
			{
				// For iOS application...
				getSession();
			...
			List<string> statusList = toStringList(response);
			return statusList;
